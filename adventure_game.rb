...
module Action
	@spells = {	:fireball => "You gain a scroll of a Fireball spell (type 'fireball' to cast)\nIt is a damage spell.",
					...
					:c_speed => "You gain a scroll of a Cheetah Speed spell (type 'cheetah speed' to cast)\nIt will increase your hit chance.",
	...
	def Action.combat_prompt
		puts "Hit Enter for next round, or cast a spell, or type 'heal' to drink potion."
	...
	def Action.heal(hero)
		if hero.spells[:heal] > 0
			...
			puts "You don't have any health potions left! That sucks!"
		end
	end
	...
	def Action.iron_skin(hero)
		if hero.spells[:iron_skin] > 0
			...
			puts "Your skin turns to iron! Rarrh!"
	...
	def Action.c_speed(hero)
		if hero.spells[:c_speed] > 0
			...
			puts "Time slows down around you and you acquire precision of a cheetah!"
...
class FirstFloor < Event
	def initialize
		...
		@riddles = {"I only point in one direction, but I guide people around the world with perfection.\nWhat am I?" => "compass",
					...
					"I have a name that is not mine, and noone cares about me in their prime.\nPeople cry at my sight and lie by me all day and night.\nWhat am I?" => "tombstone"
					}
		@riddle = rand(0..4)
	end
	...
	def enter(hero)
		...
		puts "'Hello, traveler', the goblin said. 'My name is Bob.'"
		puts "'Answer my riddle, and get a reward!'"
		puts "'Here is my riddle: #{@riddles.to_a[@riddle][0]}'"
		answer = prompt
		if answer == @riddles.to_a[@riddle][1]
			puts "'Very good, traveler!' Before you can say anything, Bob dissapears..."
			puts "The old goblin leaves behind a health potion, 3 gold coins and a scroll."
			puts "You pick up the goodies, and a spell scroll. This dungeon is easy!"
			Action.spell_table(hero)
			hero.gold = hero.gold + 3
			hero.spells[:heal] += 1
		else
			puts "'Ooh, noez, '%s' is wrong!', snarks the Old Goblin!" % answer
			puts "'Thanks for trying though, Here is 1 gold'"
			puts "The Goblin dissapears and you find a shiny coin."
			hero.gold = hero.gold + 1
		end
		line
		puts "You see two doors that appear to be leading lower into the dungeon."
		puts "The right one has a picture of a sword and shield."
		puts "The left one has a picture of a book with a skull on its cover."
...
class Library < Event
	def initialize
		@monster = Monster.new("Ghost", {hp: 10, attack_range: 1..6, ac: 12, hit_chance: 2, reward: 2})
	end

	def enter(hero)
		puts "You walk into a dark room, there are shelves everywhere with dusty books."
		puts "It looks like an abandoned library."
		puts "You feel a freezing wind, and a scary Ghost appears in front of you!"
		puts "'Prepare to die, traveler!', #{@monster.name} growls."
		Action.combat(@monster, hero)
		line
		puts "After defeating the Ghost, you catch your breath and look around."
		puts "You hastily look through the room, and find two spell scrolls and some gold!"
		Action.spell_table(hero)
		Action.spell_table(hero)
		puts "Just as you getting ready to leave, you see another sroll in the back of a dusty shelf."
		puts "The inscription says \"Dragon Charm Spell\". Interesing... It might be useful!"
...
class TreasureRoom < Event
	def initialize
		@monster = Monster.new("Goblin Archer", {hp: 15, ac: 9, crit_chance: 15, reward: 2})
	end
	...
	def enter(hero)
		...
		puts "You rush towards the archer, as he shrieks and releases an arrow!"
		if d(20) + 5 >= hero.ac
		...
		puts "You grab a longsword, a dwarven shield and a silver helmet! Nice!"
...
class SecondFloor < Event
	def initialize
		@monster = Monster.new("Skeleton Warrior", {hp: 14, attack_range: 3..5, ac: 13, crit_chance: 10, reward: 3})
	end

	def enter(hero)
		puts "You go deeper and deeper into the dungeon. You see a large silhouette that is blocking your path!"
		puts "It is a #{@monster.name} with a steel shield and a war axe. He grunts and moves towards you."
		...
		hero.spells[:heal] += 1
		Action.spell_table(hero)
		puts "Time to see what this Skeleton was guarding!"
		line
		DragonNursery.new
	end
end
...
class DragonNursery < Event
	...
	def enter(hero)
		...
		if hero.spells[:dragon_potion] == true
			puts "You better use the dragon charm spell, or you might be this baby's breakfast!"
			puts "Say 'abracadabra' precisely to cast the spell!"
		...
		puts "Mamma Dragon must have heard the commotion! You better get out of here FAST!"
...
class Shop < Event
	def initialize(hero)
		@shop = {"hammer" => [-7, 0, 5, 5, 0, 0, "The Magic Hammer"], "sword" => [-11, 0, 3, 4, 2, 10, "The Magic Sword"],
				 "shield" => [-7, 4, 0, 0, 0, 0, "The Magic Shield"], "boots" => [-5, 0, 0, 0, 4, 0, "The Magic Boots"],
				 "ring" => [-5, 0, 0, 0, 0, 20, "The Magic Ring"], "potion" => [-1, 0, 0, 0, 0, 0, "a health potion"]}
	...
	def enter(hero)
		puts "You walk into a brightly lit room, and a large lizard-looking creature waves to you!"
		puts "'Hello, traveler!', he says. 'Don't be afraid, there is no fight programmed in this room!'"
		...
		puts "1. Magic Hammer: It has super high damage! (7 gold)"
		puts "2. Magic Sword: It has good damage, improves critical and hit chance! (11 gold)"
		puts "3. Magic Shield: It will block most attacks! (7 gold)"
		puts "4. Magic Boots: You will be super fast, and you will be hard to block! (5 gold)"
		puts "5. Magic Ring: You will have a higher chance of a critical damage! (5 gold)"
		...
		puts "Thanks for shopping at \"Lizard\'s Potions and Gear\"! Have a good day! Don't die!"
...
class TrapRoom < Event
	def enter(hero)
		...
		if d(9) <= hero.wisdom
			...
			puts "I can't believe you survived this! This is so unrealistic!"
		end
		line
...
class BossLair < Event
	def initialize
		@monster = Monster.new("Pussy Demon", {hp: 50, attack_range: (3..8), ac: 12, crit_chance: 20, hit_chance: 2})
	end

	def enter(hero)
		puts "You walk into a large round room brightly lit by torches!"
		puts "It looks like an Arena! In the middle of it you see a Giant Demon!"
		puts "He is surrounded by cats!'I am a Pussy Demon!' he growls!!!"
		puts "'PREPARE TO DIE! MY CATS ARE HUNGRY!"
		line
		Action.shoot_bow(@monster, hero) if hero.bow == true
		Action.combat(@monster, hero)
		Victory.new
...
class Victory < Event
	def enter(hero)
		puts "You defeated the Boss! You will have to adopt all of his cats, I guess..."
		puts "You won! Congrats! It was not a very hard game though..."
		puts
		puts "================ G A M E    O V E R =================="
		puts "================== C R E D I T S ====================="
		puts "========  Designed and coded by Andrei Koenig  ======="
		puts "========= follow me on Twitter @andreikoenig ========="
		exit(1)
	end
end
...
class Death < Event
	def initialize
		@deaths = [ "Noone will even remember your name...\nShould have stayed home today and played board games instead.",
					"You are very bad at this...\nMay I recommend Call of Duty instead?",
...
steve = Hero.new(30 + (s * 10), 9 + (s * 2), ((1 + s)..(3 + s)), 15 + (d * 10), 0 + d, 3 + w)
